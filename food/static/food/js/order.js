...
function addRamadanShawarma(ramadanid) {
    ramadantId = '#ramadant' + ramadanid;
    var name = document.querySelector(ramadantId).innerHTML;
    var drinkOption = document.querySelector('input[name="drinkChoice' + ramadanid + '"]:checked').value;
...
function addRamadanRice(ramadanriceid, element) {
    // Navigate up to the nearest .swiper-slide container
    var container = element.closest('.swiper-slide');
    
    // Find the select element within this container
    var riceSelect = container.querySelector('select[name="rice"]');
    var riceOption = riceSelect.value;
    ramadanricetId = '#ramadanricet' + ramadanriceid;
    var name = document.querySelector(ramadanricetId).innerHTML;
    //var riceSelectId = '#id_rice' + ramadanriceid; // Make sure this id is unique if you have multiple cards
    //var riceOption = document.querySelector(riceSelectId).value;
    var foodSpice = document.querySelector('input[name="ramadanRiceSpice' + ramadanriceid + '"]:checked').value;
    var drinkOption = document.querySelector('input[name="drinkChoice' + ramadanriceid + '"]:checked').value;

    // Concatenate the selected option to the name
    name += ': ' + riceOption + ' (' + foodSpice + ' spice) and ' + drinkOption;

    ramadanricepId = '#ramadanricep' + ramadanriceid;

    var price = document.querySelector(ramadanricepId).innerHTML;
    console.log(Number(price));
    var orders = JSON.parse(localStorage.getItem('orders'));
    var total = localStorage.getItem('total');
    var cartSize = orders.length;
